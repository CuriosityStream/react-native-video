...
//        Log.i(TAG, "--- downloadContentDirectory", Error(downloadContentDirectory.toString()))
        downloadCache = SimpleCache(
                downloadContentDirectory, NoOpCacheEvictor(), getDatabaseProvider(context)!!)
//        Log.i(TAG, "--- downloadCache", Error(downloadCache.toString()))
      }
...
//      if (videoRequestModel == null || videoRequestModel?.url.isNullOrBlank() || videoRequestModel?.licenseUrl.isNullOrBlank()){
      if (videoRequestModel?.url.isNullOrBlank()){
        ret = false
      }
      return ret
    }
